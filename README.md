...
最近比赛，转发 ICPC 北京总部公告，并支持每日早报与赛前 15 分钟提醒。
...
| `icpc公告`（`ICPC公告`、`北京总部公告`） | ICPC 北京总部最新公告列表 | 所有人 |
| `最近icpc公告`（`最近北京总部公告`） | ICPC 北京总部最新一条公告 | 所有人 |
| `update`（`刷新比赛`） | 强制刷新全部平台与公告数据 | 管理员 |
| `acmer激活`（`/激活`） | 首次激活本群主动推送（重启后任意群消息自动恢复） | 所有人 |
| `acmer群管理插件菜单` | 功能菜单（`/acm菜单`、`比赛帮助` 仍可用） | 所有人 |

## ICPC 北京总部公告转发

插件会每 10 分钟检查一次 [ICPC 北京总部公告页](https://icpc.pku.edu.cn/tzgg/index.htm)，
发现**新公告**时自动转发到已启用的群：

```
📢 ICPC北京总部 新公告
🏷 第51届ICPC 亚洲区域赛（南昌）名额分配方案
🗓 2026-09-03
🔗 https://icpc.pku.edu.cn/tzgg/b4d55e13589b47d384b6c854702acc86.htm
```

- **不会刷屏**：插件首次运行、以及新群第一次被注册时，会把公告页当前的
  历史公告全部记为“已知”（建立基线），只有此后**新增**的公告才会转发。
- **不会重复**：去重记录按“群 + 公告 ID”隔离，同一条公告每个群只发一次；
  某个群发送失败时只重试该群，不会给已成功的群重复发送。
- 公告 ID 取自详情页文件名（如 `b4d55e13589b47d384b6c854702acc86`），
  站点改标题也不会重复推送。
- 全局开关与每个群的开关都在 WebUI，默认开启。
...
| ICPC 北京总部公告 | `icpc.pku.edu.cn/tzgg/index.htm` | 静态页 `ul.item-list`，正则解析 |

比赛统一转为 `Contest` 模型并缓存 5 分钟，公告转为 `Announcement` 模型并
缓存 10 分钟；网络超时 10 秒、最多重试 3 次。
...
- ICPC 北京总部公告页响应头只有 `Content-Type: text/html`、不带 charset，
  必须显式按 UTF-8 解码，否则 aiohttp 自动探测会把中文标题解成乱码。
