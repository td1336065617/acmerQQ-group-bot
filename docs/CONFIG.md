# acmerQQ群机器人 配置文档

## 1. 配置入口与存储位置

- **配置入口**：AstrBot 管理面板 → 插件管理 → **acmerQQ群机器人**
  （`pages/settings/index.html`）。
- **存储方式**：配置保存在 AstrBot 的插件 KV 存储（数据库）中，**不在插件
  目录写文件**，因此升级/重装插件不会丢配置。
- 存储键：

| 键 | 内容 |
| :--- | :--- |
| `admin_users` | 管理员 openid 列表 |
| `settings` | 全局推送设置（早报时间/平台/提醒/@全体/近期天数/转图阈值） |
| `groups` | 各群配置（自动注册） |
| `reminded` | 已提醒的比赛去重记录 |
| `morning_<群ID>_<日期>` | 当日早报是否已发送 |
| `settle_<群ID>_<平台>_<contestID>` | 赛后赛果是否已推送（每群每场一次） |
| `weekly_<群ID>_<YYYY-Www>` | 训练周报是否已推送（每群每 ISO 周一次） |
| `signup_<contestID>_<24h\|2h>` | 报名截止提醒是否已推送（全局，不按群） |
| `at_all_blocked_until_<群ID>` | @全体失败后的冷却截止时间 |
| `linked_accounts` | QQ 用户与竞赛平台账号绑定关系（不含验证码） |
| `pending_account_bindings` | 尚未完成的绑定挑战（只保存验证码哈希） |
| `group_rank_members` | 各群已加入排行的 QQ 用户 |
| `account_rating_snapshots` | 账号 Rating/排名快照，用于本周进步榜 |

## 2. 配置项一览

| 配置项 | 默认值 | 说明 |
| :--- | :--- | :--- |
| 管理员列表 `admin_users` | `[]` | 可执行 `/update` 的 openid 列表 |
| 每日早报时间 `settings.morning_push_time` | `08:00` | HH:MM（24 小时制） |
| 推送平台 `settings.push_platforms` | 牛客/CF/AtCoder/洛谷 | 早报与提醒默认平台 |
| 赛前提醒 `settings.reminder_enabled` | 开 | 开始前 15 分钟提醒 |
| 尝试 @全体成员 `settings.at_all_enabled` | 关 | 通知带 `<@everyone>`，失败自动降级 |
| 最近比赛查询天数 `settings.recent_contest_days` | `7` | `最近比赛`查看未来几天，范围 1～30 |
| 早报附今日一题 `settings.daily_problem_enabled` | 开 | 早报末尾附"今日一题"，与 `每日一题` 指令同源同一题；消息里只显示题目名与链接 |
| 每日一题平台 `settings.daily_problem_platform` | `nowcoder` | 题目来源：牛客（复用本地题库索引，零新增抓取）/ Codeforces / AtCoder / 洛谷（首次使用构建索引，7 天有效） |
| 推荐补题 `settings.recommend_enabled` | 开 | 单平台详细资料卡之后追加 3 道推荐题（未通过 + 贴合难度 + 优先薄弱知识点） |
| 赛后赛果推送 `settings.settle_push_enabled` | 开 | 比赛结束后推一张名次卡：名次 / 通过题数 / 参赛人数（**不含 Rating 变化**——CF 要等系统重测、牛客固定次日 00:00 才更新评分） |
| 赛后推送延迟 `settings.settle_delay_minutes` | `10` | 赛后多少分钟推送，范围 1～60；超过 2 小时未拿到赛果则不再补推 |
| 最少参赛人数 `settings.settle_min_participants` | `1` | 低于该人数不推送，范围 1～10 |
| 附带未通过题目 `settings.settle_show_unsolved` | 开 | 赛果卡附"本场未通过题目"，仅 Codeforces 有效 |
| 群训练周报 `settings.weekly_report_enabled` | 开 | 每周一次推「本周进步榜/退步榜」两张卡 + 文字统计（参与人数、人均活跃天数与提交数、进步/退步最多） |
| 周报推送星期 `settings.weekly_report_weekday` | `1` | `1`～`7`（周一=1），默认周一；当天该时刻触发一次 |
| 周报推送时间 `settings.weekly_report_time` | `20:00` | `HH:MM`（24 小时制，北京时间）；同一 ISO 周每群只推一次，无数据时静默跳过 |
| 牛客赛事范围 `settings.nowcoder_scope` | `all` | `all`＝全部牛客赛事（含高校校赛、新生赛、集训营、自主创建赛）；`series_only`＝仅比赛名含“牛客”的系列赛。抓取始终取全量，切换后立即生效 |
| 文字转图片最大字符数 `settings.max_plain_text_chars` | `1800` | 文字超过此字符数时转图，范围 200～10000 |
| 文字转图片最大行数 `settings.max_plain_text_lines` | `36` | 文字超过此行数时转图，范围 10～200 |
| 群启用 `groups[].enabled` | 开 | 是否向该群推送 |
| 群早报时间 `groups[].morning_push_time` | `08:00` | 覆盖全局时间 |
| 群推送平台 `groups[].push_platforms` | 全部 | 早报取“全局 ∩ 群”平台 |
| 群赛前提醒 `groups[].reminder_enabled` | 开 | 该群是否发送提醒 |

## 3. 管理员权限

- QQ 官方适配器不会把群主/管理员角色映射到 AstrBot 的管理员判断上，因此
  `/update` **必须**在 WebUI“管理员设置”里显式添加 openid，否则会提示
  “此指令仅限管理员”。
- 获取自己的 openid：在群里发一条任意消息，然后查看 AstrBot 日志中的
  `用户名/OPENID: 消息` 记录，斜杠前的就是 openid。
- 管理员代绑定 `@某人 绑定cf <账号>` 使用同一份 `admin_users`。**群主 / 群管理员身份不算管理员**：
  未加入 `admin_users` 的用户即使 @ 了成员并发送绑定指令，也只会收到“此指令仅限管理员”，
  不会触发抓取或写库。
- 这里的 `@` 必须是**真正的成员 @**（从 QQ 成员列表选择）。手打 `@昵称` 只是普通文字，
  插件拿不到对方 openid，会回复提示而不是静默。

## 3.1 后台页面分区

后台共 7 个分区：概览与状态、基础设置、推送设置、群推送配置、账号绑定、群排行、配置备份。

- **账号绑定（增删改查）**：后台可直接新增 / 换绑 / 删除绑定。与群内 `@某人 绑定cf` 共用同一份数据，
  同样跳过验证码，同样受“同一 OJ 账号全局唯一”约束（不会顶掉别人的账号）；保存 / 删除后立即失效群排行缓存。
- **群排行**：只读查看；默认命中本地快照，不触发全群抓取，点“刷新”才强制重算。
- **运行状态 + 立即试跑**：试跑仅单群，且**不写幂等键**，因此不会影响当天正式推送。
- **配置备份**：导出 / 导入 JSON；导入先校验并预览差异，确认后才写入。

## 4. 账号与推送行为

### 账号绑定与排行

- Codeforces 绑定：发送 `绑定cf <handle>`，将验证码追加到 Codeforces 的
  `Last name/姓氏`，再发送 `确认绑定cf`。
- 牛客绑定：发送 `绑定牛客 <用户ID或主页链接>`，将验证码追加到牛客
  `个性签名`，再发送 `确认绑定牛客`。
- 洛谷绑定：发送 `绑定洛谷 <UID或主页链接>`，将验证码追加到洛谷
  `个人介绍`，再发送 `确认绑定洛谷`。绑定校验读取 `luogu.com` 公开个人页；
  `.com` 暂时无法访问时兼容 `.com.cn` 资料页面。昵称、Rating、排名、题数和头像
  等其他资料同样优先读取 `.com`。
  如果个人介绍字段存在但内容为空，仍会正常发放验证码，用户追加后即可确认绑定；
  只有字段缺失或页面读取失败时才会阻止发放验证码，并给出对应提示。
- AtCoder 绑定：发送 `绑定atcoder <handle>`，将验证码追加到
  `Affiliation/所属`，再发送 `确认绑定atcoder`。
- 验证码 10 分钟有效，成功后立即失效，只保存哈希；建议在私聊中完成绑定，
  避免验证码出现在群消息里。
- 绑定成功后自动加入发起绑定群的排行；私聊绑定后，在目标群发送一次
  `加入群排行` 即可加入该群排行。发送 `退出群排行` 可退出当前群。
- `群排行` 按平台分开展示，不把不同平台 Rating 直接相加；`本周进步榜`
  根据公开 Rating 历史或最近快照计算。
- Codeforces 个人卡会显示通过题的难度分布，按题目去重，最多扫描最近 10000 条公开提交；
  区间从 `≤999` 细分到 `3200+`，如果没有可用难度字段，则计入“未标分”。
  牛客会分析公开练习页提交、题库难度/知识点和语言；AtCoder 会分析 AtCoder Problems
  估计难度、题目系列和语言；洛谷会分析公开练习页原生难度分组、题库类型、活动日历、
  资料分项评分和 Elo 历史。
  资料卡还会在有至少 2 场记录时显示最近 8 场 Rating 趋势图；这些统计只用于个人资料卡，
  不参与群排行。
- 题目级分析只在 `我的账号`、`我的战绩` 和各平台个人资料命令触发；账号分析缓存默认
  12 小时，题目资源缓存 24 小时。若平台题目数据暂时无法读取，仍会显示基础资料和
  Rating 历史，不会导致整个命令失败。
- `群cf排行`、`群牛客排行`、`群洛谷排行`、`群atcoder排行` 按每页 30 人展示，
  支持 `群cf排行 2`、`群cf排行第2页` 等页码写法；`群排行` 和
  `本周进步榜` 只展示各平台前 5 名总览，完整成员请使用对应平台排行翻页。
- 排行结果默认缓存 5 分钟；绑定、解绑、加入或退出排行，以及昵称更新后会立即
  清理相关缓存。首次刷新大群排行时会分批读取账号，并限制同时请求的平台资料数量。

### 最近比赛查询
- 群内发送 `最近比赛`（或 `近期比赛`）后，插件并发读取牛客、Codeforces、
  AtCoder、洛谷和 XCPC 线下赛缓存/数据。
- 默认展示未来 7 天内开赛或当前仍在进行的比赛；天数可在 WebUI“推送设置”
  中调整为 1～30 天，保存后立即生效。
- 该命令固定汇总全部平台，不受全局 `push_platforms` 推送平台设置影响。
- 结果按开始时间统一排序，线下赛继续显示 XCPC Link 数据源。

### 每日早报
- 到达配置时间后，汇总“当天开始”的已启用平台比赛并推送到各启用群。
- 当天没有任何比赛时**不推送**，并标记当天已检查，避免反复尝试。
- 同一群同一天只推送一次。

### 赛前提醒
- 比赛开始前 15 分钟窗口内发送提醒，按“平台+比赛ID”全局去重。
- 定时任务每 30 秒检查一次，配置修改后最多 30 秒内生效。

### @全体成员
- 开启后，早报与提醒会先尝试带 `<@everyone>` 发送。
- 若 QQ 返回失败（无权限），自动改发不带 @ 的普通通知，并把该群标记
  **6 小时内不再尝试**。
- 注意 QQ 官方机器人的 @全体成员能力受限（官方格式文档仅明确支持文字子
  频道；官方 bot 每天全群共享约 10 次），因此默认关闭。

### @全体成员为什么加了群管理也不生效

QQ 官方机器人在**群聊场景**实际不支持 `@everyone`：官方格式文档只对“文字
子频道”定义该标签，群消息接口即使接收带 `@everyone` 的内容，也不会产生
@ 效果，与机器人是否被设为群管理员无关。插件会照常发出通知，只是无法真正
@ 到人；建议在 WebUI 保持“通知尝试 @全体成员”关闭。

### 主动推送前提
- QQ 官方适配器的主动推送依赖该群**先与机器人产生过消息**（用于缓存会话
  场景）；插件在收到群消息时会自动注册该群并保持推送目标。
- 若某群从没给机器人发过消息，定时推送可能被平台侧跳过。
- 平台实例 ID（会话前缀，如“爱莉希雅”）由插件**自动获取**：发送时会动态
  查找 `qq_official` 类型平台实例，并把每个群实际来源的实例 ID 记录在该群
  配置中，无需手动填写；WebUI 会显示当前检测到的 ID 供排障。
- 若机器人本次运行期间该群还没发过任何消息，插件会在日志中提示“主动推送
  可能被 QQ 适配器跳过”，请先让群内发一条消息。
- WebUI 群配置里的“测试推送”按钮可随时验证链路：先在群里发一条消息，
  再点击按钮，机器人会立即向该群发送今日比赛早报；今日无比赛时自动改发
  最近一场比赛作为测试内容。若提示“会话未就绪”，说明该群在机器人本次
  运行期间还没发过消息，先让群里发一条消息（或发送 `acmer激活`）再试。
- 首次使用在群内发送 `acmer激活` 激活一次即可，状态会持久保存；
  AstrBot 重启后无需重复触发，群内任意一条消息都会自动重新激活
  （机器人回复激活命令时会缓存该群消息 ID，主动推送更稳定）；
  激活成功后再次发送 `acmer激活` 会被静默忽略，不再回复。

## 5. 数据源与缓存

| 平台 | 来源 | 说明 |
| :--- | :--- | :--- |
| Codeforces | `codeforces.com/api/contest.list` | 官方 API，`phase=BEFORE` |
| 牛客 | `ac.nowcoder.com/acm/calendar/contest` | 牛客比赛日历，抓当前月及后续 3 个月；按 `ojName` 判定 OJ 归属（`NowCoder`），除系列赛外还收录高校校赛/新生赛同步赛/集训营等 |
| AtCoder | `atcoder.jp/contests/` | 官网 Upcoming Contests 赛程表 |
| 洛谷 | `luogu.com.cn/contest/list` | 页面内 `#lentille-context` JSON，无需登录 |

账号分析数据源：

| 平台 | 来源 | 分析内容 |
| :--- | :--- | :--- |
| Codeforces | 官方 `user.status` API | 通过题难度、提交通过率、语言、活跃度 |
| 牛客 | 公开练习页（最多 200 条/页 × 100 页 ＝ 20000 条）+ 牛客题库索引 | 题目难度、知识点、提交通过率、语言、活跃度；题库索引整库抓取（约 1.4 万题，每周更新并落盘 `nowcoder_problem_index.json`），覆盖全部通过题 |
| 洛谷 | 公开个人页/练习页 `#lentille-context` | 原生难度分组、题库类型、活动日历、资料分项评分、Elo 历史 |
| AtCoder | AtCoder Problems 公开资源 | 提交记录、估计难度、题目系列、语言、活跃度 |
| 线下赛 | `xcpc.link`（备用 `xcpc.ink`） | 首页脚本中的线下赛程数据 |

- 在线平台使用 5 分钟内存缓存；线下赛使用 30 分钟内存缓存，并持久化到插件
  `data/contest_cache.json`，插件重载后也能复用。
- 缓存过期时同一平台的并发查询会合并为一次抓取；网络临时失败时，如果存在旧缓存
  会继续返回旧数据，避免网站短暂波动导致查询失败。
- `/update`（管理员）可强制刷新全部平台；线下赛首次抓取会并行读取动态 JS chunk，
  不依赖 Chrome。
- 网络超时 10 秒，最多重试 3 次；失败返回友好提示并记录日志。

## 6. 长消息输出

- 菜单、比赛列表、线下赛列表、刷新结果、早报和提醒会按长度自动选择输出方式：
  短内容发送文字，长内容写入 HTML 后转为 PNG 图片发送。
- “短/长”的判断由 WebUI“推送设置”中的 `max_plain_text_chars` 和
  `max_plain_text_lines` 控制；两个条件只要超过一个就转图。保存设置后立即生效，
  不需要重启插件。
- 图片缓存于插件 `data/output_cache/`；相同内容直接复用，内容变化后才重新生成。
- 转图支持 Chromium/Chrome、Firefox、`wkhtmltoimage`；没有浏览器时使用 Pillow。
  如果运行环境没有可用渲染器，则自动拆分为多条文字，不会丢失结果。
- 中文字体会优先选择 `Noto Sans CJK SC` 的简体中文字体面；如需指定字体文件，
  可设置环境变量 `ACMER_QQ_BOT_FONT`，使用 TTC 字体时可通过
  `ACMER_QQ_BOT_FONT_INDEX` 指定字体面索引（默认自动选择简体中文面）。
- Emoji 使用插件自带 `assets/fonts/NotoColorEmoji.ttf`，不依赖服务器安装
  Emoji 字体。HTML 渲染会加载插件目录内的字体文件；Pillow 回退会先高分辨率
  绘制再缩放，字体无法加载时使用安全符号，避免出现方框字形。如需覆盖字体，
  可设置 `ACMER_QQ_BOT_EMOJI_FONT`，使用 TTC 字体时可通过
  `ACMER_QQ_BOT_EMOJI_FONT_INDEX` 指定字体面索引。
- 菜单和长消息图片采用较宽的字间距、行距与条目留白，避免中文内容过密；
  相关排版调整会通过缓存版本自动生效。
- `我的账号`、`我的战绩` 和各平台个人战绩卡使用独立的 HTML/PNG 模板；
  单平台卡片使用紧凑整行布局，多平台卡片按平台数量自适应高度，减少底部空白。
- 资料卡会根据公开 Rating 历史显示趋势折线图，Codeforces 账号会显示通过题难度条形图；
  浏览器不可用时，Pillow 回退渲染也保留这些图表。
- 群聊发送个人资料命令时，卡片会显示当前用户的对应平台本群排行；私聊没有群上下文，
  因此不会显示群排行。头像缩略图固定为正方形并裁剪填充，头像读取失败时使用平台色占位图。
- 账号卡、群排行卡、长消息图片和后台配置页使用统一的樱粉/珍珠白/冰蓝主题；
  Pillow 回退渲染也会沿用该主题，不要求安装 Google Chrome。

## 7. 常见问题

**`/update` 提示“此指令仅限管理员”**：管理员列表为空或未包含你的 openid，
到 WebUI 添加后重试。

**插件已安装、后台能配置，但指令/群自动注册都没反应**：这是 AstrBot 的
“可用插件”名单（`plugin_set`）把本插件过滤掉了。检查 AstrBot
**设置 → 插件配置 → 可用插件**是否包含 `acmer_qq_group_bot`（或设为
“全部”），并检查该群/会话级插件开关是否启用了本插件；修改后保存并重载。

**没收到早报/提醒**：依次检查——群是否出现在 WebUI 群列表且已启用；该群
是否给机器人发过消息（主动推送前提）；当天/该时段是否有比赛；提醒开关是否
开启；@全体开启时是否失败降级（会有一条普通通知）。

**日志提示“QQOfficial No cached msg_id ... skip send_by_session”**：这是
QQ 官方适配器的主动推送限制——需要该群在机器人**本次运行期间**发过消息来
缓存会话场景。让群内随便发一条消息后再等待下一个推送周期即可。

**@全体成员没生效**：QQ 官方机器人在群聊的 @全体能力受限，失败后插件已
自动降级为普通通知，并在 6 小时内不再尝试。

**配置改了多久生效**：保存立即生效；定时任务最多 30 秒内按新配置执行。

## 7. 不建议直接改数据库

所有配置应通过 WebUI 修改。直接改 AstrBot 数据库的 KV 记录有一定风险
（格式校验、并发写入），仅在排障时由维护者谨慎操作。

## 8. 账号存储与回滚（1.3.0）

- 账号/群成员/待确认绑定/Rating 快照默认存储在本地 SQLite：
  `data/plugin_data/acmer_qq_group_bot/acmer_store.db`。
- 首次启用 SQLite 时会自动从 AstrBot KV 迁移，并在同目录生成 JSON 备份
  （`kv_backup_*.json`）。
- 默认开启 KV 双写；如需完全回退到旧 KV 模式，设置环境变量
  `ACMER_STORE_BACKEND=kv` 后重启插件。SQLite 数据不会删除，切回
  `sqlite` 可继续使用。
- 迁移失败不会阻断插件启动，会自动回退 KV 模式并在日志中记录原因。
- 相关环境变量：`ACMER_STORE_BACKEND`、`ACMER_STORE_DIR`、
  `ACMER_DUAL_WRITE_KV`。
